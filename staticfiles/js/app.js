    ...
    container.innerHTML = sorted.slice(0, 15).map(r => `
      <div class="road-item" style="border-left-color:${getFlowColor(r.daily_flow)}">
        ...
        <div class="road-meta">
          ...
          <span>EV占比 ${(r.ev_ratio*100).toFixed(0)}%</span>
        </div>
      </div>`).join('');
  } catch (e) { console.error(e); }
}
        ...
        <div class="report-section-body">
          ...
          <table class="data-table">
            ...
            <tbody>${traffic.items.slice(0,6).map(r => `
              <tr><td>${r.road_name}</td><td>${r.road_level}</td><td>${r.distance_km}</td><td style="color:#f59e0b;font-weight:600">${r.daily_flow?.toLocaleString()}</td><td>${(r.ev_ratio*100).toFixed(0)}%</td></tr>`).join('')}
            </tbody>
          </table>` : ''}
        </div>
      </div>
